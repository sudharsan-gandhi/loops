...
@Injectable()
export class GrantDTOAuthorizer implements CustomAuthorizer<GrantDTO> {
  NAME = 'grant';
  OWNERKEY = 'postedById';
  ...
  async authorize(
    ...
    if (allowed) {
      ...
      const resource = await Grant.findOne(resourceId);
