import { PaymentInputDTO } from 'src/_dto';
...
@Injectable()
export class PaymentAuthorizer implements CustomAuthorizer<PaymentInputDTO> {
  NAME = 'payment';
  OWNERKEY = 'userId';
  ...
  async authorize(
  ...
  ): Promise<Filter<PaymentInputDTO>> {
    ...
    if (allowed) {
      ...
      const resource = await Payment.findOne(resourceId);
